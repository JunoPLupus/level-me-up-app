...
describe('TaskUiConstants', () => {
    ...
    describe('Priority utilities', () => {
      ...
      describe('getSliderValueFromPriority', () => {
    ...
    describe('Tag Utilities', () => {
      ...
      describe('getTagName', () => {
        it('should return the correct tag name for a valid ID', () => {
          const validTag = MOCK_TAGS[0];
          const result = getTagName(validTag.id);

          expect(result).toBe(validTag.name);
        });

        it('should return "Others" for an invalid or empty ID', () => {
          expect(getTagName('id-that-dont-exist')).toBe('Others');
          expect(getTagName('')).toBe('Others');
        });
      });

      describe('getTagColor', () => {
        it('should return the correct hex color for a valid ID', () => {
          const validTag = MOCK_TAGS[0];
          const result = getTagColor(validTag.id);

          expect(result).toBe(validTag.colorHex);
        });

        it('should return default gray (#808080) for invalid ID', () => {
          expect(getTagColor('ghost-id')).toBe('#808080');
        });
      });
